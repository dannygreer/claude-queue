#!/usr/bin/env python3
"""Two-project isolation tests for qw + taskwatch.

Guards the invariant that broke on 2026-08-05: launching `qw` in a fresh
project (no TASKS.md yet) must bind to THAT project and wait — never open
...
def run_qw(args, cwd, memory, extra_env=None):
    """Run qw with taskwatch replaced by /bin/echo so execv prints its argv.
...
def run_taskwatch(args, timeout=30):
...
class TwoProjects(unittest.TestCase):
    """Project A: several completed tasks. Project B: starts with no TASKS.md."""
    ...
    def setUp(self):
        ...
        self.proj_a.mkdir()
        self.proj_b.mkdir()
        (self.proj_a / "TASKS.md").write_text(
            "# Claude Task Queue\n\n"
            ...
            encoding="utf-8")
        ql.refresh(self.proj_a, self.proj_a / "TASKS.md", session_id="sessA")
    ...
    def test_plain_qw_in_empty_project_never_opens_remembered(self):
        ...
        self.assertEqual(out, str(self.proj_b / "TASKS.md"))
    ...
    def test_resolution_prefers_tasks_md_ancestor(self):
        ...
        self.assertEqual(rc, 0, err)
        self.assertEqual(out, str(self.proj_a / "TASKS.md"))
    ...
    def test_taskwatch_waits_for_missing_tasks_md_and_shows_no_foreign_tasks(self):
        rc, out, err = run_taskwatch([str(self.proj_b / "TASKS.md"), "--once"])
        self.assertEqual(rc, 0, err)
        self.assertIn("Waiting for this project's TASKS.md", out)
    ...
    def test_taskwatch_shows_queue_once_file_appears(self):
        (self.proj_b / "TASKS.md").write_text(
            "# Claude Task Queue\n\n- [ ] B-task-one delta\n", encoding="utf-8")
        rc, out, err = run_taskwatch([str(self.proj_b / "TASKS.md"), "--once"])
        self.assertEqual(rc, 0, err)
    ...
    def test_project_header_lines_present(self):
        rc, out, _ = run_taskwatch([str(self.proj_a / "TASKS.md"), "--once"])
        self.assertEqual(rc, 0)
    ...
    def test_concurrent_edits_stay_isolated(self):
        ...
        ql.refresh(self.proj_b, self.proj_b / "TASKS.md", session_id="sessB")
        ql.capture_prompt(self.proj_b, "prompt only for project B", "sessB", str(self.proj_b))
        a_md = self.proj_a / "TASKS.md"
        ...
        _, out_a, _ = run_taskwatch([str(a_md), "--once"])
        _, out_b, _ = run_taskwatch([str(self.proj_b / "TASKS.md"), "--once"])
    ...
    def test_state_files_live_inside_each_project(self):
        for proj in (self.proj_a,):
            ...
            self.assertEqual(sd, proj / ".claude" / "taskwatch")
            self.assertTrue(ql.state_path(proj).exists())

    def test_json_snapshot_reports_bound_project(self):
        rc, out, _ = run_taskwatch([str(self.proj_a / "TASKS.md"), "--json"])
        snap = json.loads(out)
        self.assertEqual(snap["project"], str(self.proj_a))
        rc, out, _ = run_taskwatch([str(self.proj_b / "TASKS.md"), "--json"])
        snap = json.loads(out)
...
class HomeFallback(unittest.TestCase):
    ...
    def test_home_uses_remembered_only_when_home_not_a_project(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            proj.mkdir()
            (proj / "TASKS.md").write_text("# Claude Task Queue\n", encoding="utf-8")
            ...
            self.assertEqual(rc, 0, err)
            self.assertEqual(out, str(proj / "TASKS.md"))
            # but if $HOME is itself a project, bind to $HOME instead
            (fake_home / "TASKS.md").write_text("# Claude Task Queue\n", encoding="utf-8")
            rc, out, err = run_qw([], fake_home, memory, extra_env={"HOME": str(fake_home)})
            self.assertEqual(rc, 0, err)
            self.assertEqual(out, str(fake_home / "TASKS.md"))
